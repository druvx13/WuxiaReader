 ...
 *   "h2".textContent
 */
function allnovel_fetch_chapter_content(HttpClient $http, string $url, float $throttle = ALLNOVEL_MINIMUM_THROTTLE): array {
 ...
 */
function allnovel_parse_novel_page(HttpClient $http, string $url, float $throttle = ALLNOVEL_MINIMUM_THROTTLE): array {
    if ($throttle < ALLNOVEL_MINIMUM_THROTTLE) {
        $throttle = ALLNOVEL_MINIMUM_THROTTLE;
    }
    $html = $http->get($url);
...
function allnovel_import_to_db(
    ...
    $novel = allnovel_parse_novel_page($http, $url, $throttle);
    ...
    try {
        ...
        for ($i = $startIdx; $i <= $endIdx; $i++, $orderIndex++) {
            ...
            if (trim($contentHtml) === '') {
                $res = allnovel_fetch_chapter_content($http, $ch['url'], $throttle);
                if (!empty($res['title'])) {
                    $ch['name'] = $res['title'];
                }
