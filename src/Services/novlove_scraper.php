 ...
 */
function novlove_parse_novel_page(HttpClient $http, string $url, float $throttle = NOVLOVE_MINIMUM_THROTTLE, ?callable $log = null): array {
 ...
 */
function novlove_fetch_chapter_content(HttpClient $http, string $url, float $throttle = NOVLOVE_MINIMUM_THROTTLE): array {
    if ($throttle < NOVLOVE_MINIMUM_THROTTLE) {
        $throttle = NOVLOVE_MINIMUM_THROTTLE;
    }
...
function novlove_import_to_db(
    ...
    $novel = novlove_parse_novel_page($http, $url, $throttle, $logger);
    ...
    try {
        ...
        for ($i = $startIdx; $i <= $endIdx; $i++, $orderIndex++) {
            ...
            $res = novlove_fetch_chapter_content($http, $ch['url'], $throttle);
            if (!empty($res['title'])) {
                $ch['name'] = $res['title'];
            }
